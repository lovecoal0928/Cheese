...
export const useDijkstra = () => {
    ...
    function getShortestPath(graph: { [key: string]: { [key: string]: number } }): string[] {
        const nodes = Object.keys(graph);
    ...
    function getPermutations(nodes: string[]): string[][] {
        if (nodes.length === 0) {
            return [[]];
        }
        const result: string[][] = [];
    ...
    return { getAllPaths, getShortestPath, getPermutations }
}
