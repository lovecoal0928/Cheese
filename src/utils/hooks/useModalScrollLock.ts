...
export const useModalScrollLock = (args: UseModalScrollLockArgs) => {
  ...
  const scrollLock = (event: TouchEvent) => {
    const canScrollElement = (event.target as HTMLDivElement)?.closest(`#overlay`);
  ...
  const scrollLockFixAdd = (element: HTMLElement) => {
    const canScrollElement = element.querySelector<HTMLDivElement>(`#overlay`);
  ...
  const scrollStopBackContent = () => {
    const canScrollModalElement = document.querySelector<HTMLDivElement>(`#overlay`);

    if (canScrollModalElement === null || !isZoom) return;
  ...
  useEffect(scrollStopBackContent, [isZoom]);
};